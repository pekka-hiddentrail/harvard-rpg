 ...
 *
 * Averages every non-lecture slot for a course code — section, lab, or seminar, whichever
 * shape that course's "which instance did I land in" choice takes (Expos 20's real slots
 * are `type: seminar`, not `section`) — when they're all the same length (the common case;
 * CS50's six sections are all 165 minutes), that average is exact. Lecture is excluded
 * because it's the one shared component every section already gets on top of, not the
 * registration-time choice this is sizing. Once a player has actually registered, feed
 * that slot's own length in instead — the estimate is meant to sharpen after shopping
 * week, not just once, ever.
 */
export function representativeSectionHours(courseCode: string, slots: readonly CourseSlot[]): number {
  const sections = slots.filter((s) => s.courseCode === courseCode && s.type !== 'lecture')
